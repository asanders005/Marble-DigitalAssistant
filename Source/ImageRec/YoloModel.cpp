...
bool YOLOModel::Initialize(const std::string &modelPath, const std::string &labelsPath, int threadCount)
{
    ...
    if (!loadModel(modelPath)) {
        std::cerr << "YOLOModel: Failed to load model: " << modelPath << std::endl;
        return false;
    }
    if (!labelsPath.empty()) loadLabels(labelsPath);
    return true;
}
...
bool YOLOModel::loadLabels(const std::string &labelsPath)
{
    ...
    if (!content.empty() && content.front() == '[') {
        ...
        while (true) {
            ...
            labels.push_back(content.substr(pos + 1, end - pos - 1));
            pos = end + 1;
        }
    } else {
...
std::vector<YoloDetection> YOLOModel::detect(const cv::Mat &img, float confThresh, float iouThresh)
{
    ...
    if (confThresh < 1e-4f) confThresh = 0.5f;
    if (iouThresh < 1e-4f) iouThresh = 0.5f;
    ...
    cv::Mat blob = cv::dnn::blobFromImage(resized, 1.0/255.0, inputSize, cv::Scalar(), true, false);
    try {
        ...
        float rawMaxConf = 0.f; float sigMaxConf = 0.f; int countOver001 = 0; int countOver01 = 0; int countOverThresh = 0;
        ...
        for (int i = 0; i < num_preds; ++i) {
            for (int r = 0; r < attrs; ++r) {
                ...
                if (v < minAttr[r]) minAttr[r] = v;
                if (v > maxAttr[r]) maxAttr[r] = v;
            }
            float rawConf = a.at<float>(4, i);
            float conf = 1.0f / (1.0f + std::exp(-rawConf)); // sigmoid on logits
            if (rawConf > rawMaxConf) rawMaxConf = rawConf;
            if (conf > sigMaxConf) sigMaxConf = conf;
            if (conf > 0.001f) ++countOver001;
            if (conf > 0.01f) ++countOver01;
            if (conf > confThresh) ++countOverThresh;
        }
        std::cerr << std::fixed << std::setprecision(6);
        std::cerr << "YOLOModel::detect: attrs="<<attrs<<" num_preds="<<num_preds
                  <<" rawMaxConf="<<rawMaxConf
                  <<" sigMaxConf="<<sigMaxConf
                  <<" >0.001="<<countOver001
                  <<" >0.01="<<countOver01
                  <<" >"<<confThresh<<"="<<countOverThresh
                  <<std::endl;
        for (int r = 0; r < attrs; ++r) {
            std::cerr << " attr["<<r<<"] min="<<minAttr[r]<<" max="<<maxAttr[r]<<std::endl;
        }

        // Print top-k predictions by confidence (useful to inspect units and values)
        int topk = std::min(10, num_preds);
        std::vector<std::pair<float,int>> confIdx; confIdx.reserve(num_preds);
        for (int i = 0; i < num_preds; ++i) {
            float raw = a.at<float>(4,i);
            float s = 1.0f / (1.0f + std::exp(-raw));
            confIdx.emplace_back(s, i);
        }
        std::sort(confIdx.begin(), confIdx.end(), [](const auto &A, const auto &B){ return A.first > B.first; });
        std::cerr << "Top "<< topk <<" predictions by confidence:" << std::endl;
        for (int ti = 0; ti < topk; ++ti) {
            int i = confIdx[ti].second;
            std::ostringstream ss;
            ss << " top["<<ti<<"] idx="<<i<<" conf="<<confIdx[ti].first<<" ->";
            for (int r = 0; r < std::min(attrs, 8); ++r) ss << " " << a.at<float>(r,i);
        ...
        for (int r = 0; r < std::min((int)maxAttr.size(), attrs); ++r) {
            if (maxAttr[r] > 1.5f) { coordsArePixels = true; break; }
        }
        if (coordsArePixels) std::cerr << "YOLOModel::detect: interpreting box coords as pixel units" << std::endl;
        ...
        for (int i = 0; i < num_preds; ++i) {
            ...
            if (attrs > 5) {
                ...
                if (sum > 0.0f) {
                    for (int c = 0; c < numClasses; ++c) {
                        cls[c] /= sum;
                        if (cls[c] > bestProb) { bestProb = cls[c]; best = c; }
                    }
                }
                predictedClass = best;
            ...
            if (labels.size() > 1) {
                ...
                int personIdx = -1;
                for (size_t li = 0; li < labels.size(); ++li) if (labels[li] == "person") { personIdx = (int)li; break; }
                if (personIdx != -1 && predictedClass != personIdx) continue; // skip non-person
            }
        ...
        for (size_t i = 0; i < boxes.size(); ++i) {
            ...
            if ((float)b.area() < minArea) continue;
            if (b.height < minH) continue;
            if (b.width > 0 && (b.height / (float)b.width) < 0.6f) continue;
            scoredBoxes.emplace_back(scores[i], b, preNmsIdx[i]);
        }
        // Sort by score descending and keep only top 50 before NMS
        std::sort(scoredBoxes.begin(), scoredBoxes.end(), [](const auto &A, const auto &B){ return std::get<0>(A) > std::get<0>(B); });
        if (scoredBoxes.size() > 50) scoredBoxes.resize(50);
        ...
        if (boxes.empty()) {
            ...
            if (maxAttr[0] > inpW || maxAttr[1] > inpH) usedPixelFallback = true;
        }
        ...
        if (idxs.size() > 10) idxs.resize(10);
        ...
        if (idxs.empty() && sigMaxConf > 0.0f && sigMaxConf < confThresh) {
            ...
            std::cerr << "YOLOModel::detect: no results with conf="<<confThresh
            ...
            if (!dbgBoxes.empty()) {
                ...
                std::cerr << "YOLOModel::detect: debug pass found "<<dbgIdxs.size() <<" boxes" << std::endl;
            }
        }
        ...
        if (dets.size() > 1) {
            ...
            std::vector<int> parent(dets.size()); for (size_t i=0;i<dets.size();++i) parent[i]=i;
            std::function<int(int)> findp = [&](int a)->int{ return parent[a]==a?a:parent[a]=findp(parent[a]); };
            auto unite = [&](int a,int b){ a=findp(a); b=findp(b); if (a!=b) parent[b]=a; };

            for (size_t i = 0; i < dets.size(); ++i) {
                for (size_t j = i+1; j < dets.size(); ++j) {
                    float u = iou(dets[i].box, dets[j].box);
                    if (u > 0.3f) unite((int)i,(int)j);
                }
            }

            // collect clusters
            std::unordered_map<int, std::vector<int>> clusters;
            for (size_t i=0;i<dets.size();++i) clusters[findp((int)i)].push_back((int)i);

            std::vector<YoloDetection> merged;
            for (auto &kv : clusters) {
                const std::vector<int> &idxsCl = kv.second;
                if (idxsCl.empty()) continue;
                ...
                if (nb.width <=0 || nb.height<=0 || areaFrac > 0.6f) continue;
                YoloDetection md; md.box = nb; md.score = bestScore; md.classId = 0; md.trackId = -1;
                merged.push_back(md);
            }
            dets.swap(merged);
        ...
        if (dets.size() > kMaxDetections) {
            std::sort(dets.begin(), dets.end(), [](const YoloDetection &A, const YoloDetection &B){
        ...
        for (const auto &kv : trackLastSeen) {
            int tid = kv.first;
            int last = kv.second;
            if (matchedTracks.find(tid) != matchedTracks.end()) continue;
